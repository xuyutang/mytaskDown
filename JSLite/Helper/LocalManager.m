...
static NSString* CREATE_TABLE_TASK_DOWN = @"create table if not exists task_down (descripStr text,dutyer varchar(50),startTime varchar(50),endTime varchar(50),remarkStr text )";
...
- (void)_initTable {
    ...
    [self _createTable:CREATE_TABLE_TASK_DOWN];
...
- (NSMutableArray*) getProvinces{
    NSMutableArray* result = [[[NSMutableArray alloc]init]autorelease];
    NSString *filePath = [[NSBundle JSLiteResourcesBundle] pathForResource:@"city" ofType:@"db"];
...
//任务下发
-(BOOL)saveTaskDown:(NSMutableArray*)taskMuarray {
    FMDatabase* db = [FMDatabase databaseWithPath:dbPath];
    if ([db open])
    {
        [db beginTransaction];
        [db executeUpdate:@"delete from task_down;"];
        for (taskDownModel *model in  taskMuarray) {
             [db executeUpdate:[NSString stringWithFormat:@"insert into task_down (descripStr,dutyer,startTime,endTime,remarkStr) values ('%@','%@','%@','%@','%@');",model.descripStr,model.dutyer,model.startTime,model.endTime,model.remarkStr]];
        }
...
-(NSMutableArray*)getTasks {
    ...
    if ([db open])
    {
        NSString * sql = @"select * from task_down";
        FMResultSet * rs = [db executeQuery:sql];
        while ([rs next])
        {
            taskDownModel* model = [[taskDownModel alloc] init];
            
            model.descripStr =[rs stringForColumn:@"descripStr"] ;
            model.dutyer =[rs stringForColumn:@"dutyer"] ;
            model.startTime = [rs stringForColumn:@"startTime"];
            model.endTime = [rs stringForColumn:@"endTime"];
            model.remarkStr =[rs stringForColumn:@"remarkStr"];
